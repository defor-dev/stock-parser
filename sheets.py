...
class Spreadsheet:
    def __init__(self, jsonKeyFileName, sheetId = 0, sheetTitle = 'stocksheet', debugMode = False):
    ...
    def create(self, title, sheetTitle, rows = 10000, cols = 3, locale = 'en_US', timeZone = 'Etc/GMT'):
    ...
    def setSpreadsheetById(self, spreadsheetId, sheet=0):
        spreadsheet = self.service.spreadsheets().get(spreadsheetId = spreadsheetId).execute()
        if self.debugMode:
            pprint(spreadsheet)
        self.spreadsheetId = spreadsheet['spreadsheetId']
        self.sheetId = spreadsheet['sheets'][sheet]['properties']['sheetId']
        self.sheetTitle = spreadsheet['sheets'][sheet]['properties']['title']
